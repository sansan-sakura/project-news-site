...
// for HTML elements
const card = document.querySelector(".card-contents-wrapper");
...
function callApiCurrentWeather(city) {
  fetch(
    ...
    .then((data) => {
      const weatherArr = data.list.filter((obj) => obj.dt_txt.includes("21:00"));
...
function createCardUpper(obj, cityName, sunrise, sunset, timeNow) {
  ...
  let htmlElm = `
    ...
    <h1  ${main === "Clouds" ? `class="dark-text"` : ""}>${temp.toFixed(
    1
  )}<span class="degree-large">&#x2103;</span></h1>

    <div>
     <img src=${iconurl} class="icon-main">
      <p>${temp_max.toFixed(1)}<span class="degree">&#x2103;</span>/${temp_min.toFixed(
    1
...
function createCardUnder(arr) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    <span class="min-max-temp-under-box">${temp_max.toFixed(1)}&#176;/${temp_min.toFixed(
      1
    )}&#176;</span></p>
      </div>
     `;
...
function getCityName() {
  ...
  const finalValue = firstLetter + value.slice(1);
  card.textContent = "";
  inputHolder.finalValue = "";
  callApiCurrentWeather(finalValue);
  closeModal();
  value && inputHolder.value ? (inputHolder.value = "") : (inputNoitem.value = "");
}
...
function openModal() {
  ...
  clickBtnL.style.zIndex = "-1";
  clickBtnR.style.zIndex = "-1";
}
...
function closeModal() {
  ...
  clickBtnL.style.zIndex = "10000";
  clickBtnR.style.zIndex = "10000";
}
